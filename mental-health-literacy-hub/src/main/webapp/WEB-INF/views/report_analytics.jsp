    ...
    <div class="row g-4 mb-5">
        <div class="col-md-4">
            <div class="card shadow-sm text-center p-3">
                ...
                <p class="display-6 fw-bold">${usersEngaged}</p>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card shadow-sm text-center p-3">
                <h5 class="card-title">Resources Used</h5>
                <p class="display-6 fw-bold">${resourcesUsed}</p>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card shadow-sm text-center p-3">
                <h5 class="card-title">Feedback Received</h5>
                <p class="display-6 fw-bold">${feedbackCount}</p>
            </div>
        </div>
    </div>
...
<script>
  ...
  const feedbackSummary = JSON.parse('${feedbackSummaryJson}');
  const feedbackLabels = Object.keys(feedbackSummary);
  const feedbackData = Object.values(feedbackSummary);
  ...
  // === Users Engaged Chart ===
  const usersEngaged = ${usersEngaged};
  new Chart(document.getElementById('userEngagementChart'), {
    type: 'line',
    data: {
      labels: ['Users Engaged'], // for now single point
      datasets: [{
        label: 'Active Users',
        data: [usersEngaged],
  ...
  const resourcesUsed = ${resourcesUsed};
  new Chart(document.getElementById('resourceUsageChart'), {
    type: 'bar',
    data: {
      labels: ['Resources Used'],
      datasets: [{
        label: 'Usage Count',
        data: [resourcesUsed],
