...
public class MentalHealthChallengeController {
  ...
  public String submitDailyReflection(@RequestParam String reflection, @RequestParam String feeling,
  ...
  public ModelAndView manageDetailChallenge(@PathVariable("id") int id, HttpSession session) {
    ...
    // Verify ownership
    if (challengeType != null && challengeType.getCreator().equals(getUserFromSession(session))) {
  ...
  public String createChallenge(@RequestParam String title, @RequestParam String description,
      @RequestParam int totalDays, @RequestParam Set<String> activities, HttpSession session) {
    ...
    MentalHealthChallengeType challengeType = new MentalHealthChallengeType().setTitle(title)
        .setDescription(description).setTotalDays(totalDays)
        .setActivities(new LinkedHashSet(activities)).setCreator(getUserFromSession(session));
    ...
    for (String user : users) {
      this.challengeDAO
          .saveOrUpdate(new MentalHealthChallenge().setTChallengeType(challengeType).setUser(user));
    }
    return "redirect:/mental-health-challenge/";
  }
